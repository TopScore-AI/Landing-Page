...
const cards = [
  { icon: "play", title: "Interactive Quiz", desc: "Practice quizzes and self-assessment for KCSE.", color: "bg-indigo-50 text-brand-primary", dark: false, delay: 0.4, float: "y", col: 0 },
  { icon: "sparkles", title: "Multimodal Live", desc: "AI camera vision for step-by-step problem solving.", color: "bg-slate-900 text-white", dark: true, delay: 0.6, float: "y", col: 0 },
  { icon: "pdf", title: "AI PDF Reader", desc: "Synced PDF parsing powered by AI indexing.", color: "bg-emerald-50 text-emerald-600", dark: false, delay: 0.5, float: "x", col: 1 },
  { icon: "flash", title: "Smart Flashcards", desc: "Automatic spaced repetition based on curriculum depth.", color: "bg-rose-50 text-rose-600", dark: false, delay: 0.7, float: "x", col: 1 },
]
      ...
      <div class="flex flex-col lg:flex-row items-center gap-20">
        <div class="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left">
          ...
          <Motion as="p" :initial="{ opacity: 0, y: 20 }" :animate="{ opacity: 1, y: 0 }"
            ...
            class="text-xl md:text-2xl text-slate-500 leading-relaxed max-w-2xl mb-12 font-medium">
            {{ t('hero.sub') }}
          </Motion>
          ...
          <Motion :initial="{ opacity: 0, y: 20 }" :animate="{ opacity: 1, y: 0 }"
            ...
            <a href="#features"
              class="w-full sm:w-auto bg-white border border-slate-200 text-slate-900 px-10 py-5 rounded-[2rem] font-black text-sm uppercase tracking-widest hover:bg-slate-50 hover:border-slate-300 transition-all text-center focus:outline-none focus:ring-4 focus:ring-slate-200"
          ...
          <Motion :initial="{ opacity: 0 }" :animate="{ opacity: 1 }" :transition="{ duration: 1, delay: 0.6 }"
            ...
            <a href="#" target="_blank" rel="noopener noreferrer">
              <img src="/app-store-badge.svg" alt="App Store" class="h-8 w-auto" >
            </a>
          </Motion>
        </div>
